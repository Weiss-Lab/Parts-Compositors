...
function [] = CheckPlotAxisValues(axes_handle, line_index, axis, ...
    ...
    have_correct_vals = all(le(abs(correct - plot_vals), tolerance * correct));
    assert(have_correct_vals, ...
           sprintf('Check your %s values for line %d.', axis, line_index));
end
...
function [] = CheckPlotAxesLabels(axes_handle, x_label, y_label)
    ...
    assert(correct_x_label, 'Check your ylabel string');
end
...
function [] = CheckBioSystemInitialConditions(sys, compositors, concentrations)
    ...
    for i = 1:length(compositors)
        assert(sys.compositors(sys.map_compositors(compositors{i})).init_value == ...
            concentrations(i), ...
...
function [] = CheckBioSystemParts(sys, correct_sys)
    ...
    assert(length(sys.parts) == length(correct_sys.parts), ...
        'You don''thave the right number of parts defined!')
    for i = 1:length(sys.parts)
        % Check number of rates for each part
        assert(length(sys.parts(i).rates) == length(correct_sys.parts(i).rates), ...
            'You don''t have the right number of rates for part %d!', i)
        for j = length(sys.parts(i).rates)
            % Check rate equivalence -- sym conversion auto-simplifies
            assert(isequal( ...
                sym(sys.parts(i).rates(j).string), ...
                sym(correct_sys.parts(i).rates(j).string)), ...
                'You don''t have the right rate for part %d rate %d!', i, j)
        end
    end
end
